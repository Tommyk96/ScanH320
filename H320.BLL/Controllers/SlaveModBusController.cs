...
{
    public class SlaveModBusController: IBusControl
    {
        private enum DO 
        {
            GreenLight = 1,
        ...
        public SlaveModBusController( IModBusState modBusState, string ip,byte slaveAddress, ushort inStartAddress, ushort outStartAddress ) 
        {
        ...
        public Task<bool> OnGreenLight()
        {
            return Task.Run(async () =>
            {
                _outputs[(int)DO.GreenLight] = true;
                _waitEvent.Set();
        ...
        public Task<bool> OffGreenLight()
        {
            return Task.Run(async () =>
            {
                _outputs[(int)DO.GreenLight] = false;
                _waitEvent.Set();
                await Task.Delay(100);
                return true;
            });
        ...
        public Task<bool> StartGreenSpot()
        {
            return Task.Run(async () =>
            {
                _outputs[(int)DO.GreenSpot] = true;
                _waitEvent.Set();

                await Task.Delay(1000);

                _outputs[(int)DO.GreenSpot] = false;
                _waitEvent.Set();
                return true;
            });
        }
        ...
        public Task<bool> SoundAlarmBeep()
        {
            return Task.Run(async () =>
            {
                _outputs[(int)DO.Sound] = true;
                _waitEvent.Set();

                await Task.Delay(700);

                _outputs[(int)DO.Sound] = false;
                _waitEvent.Set();
                return true;
            });
