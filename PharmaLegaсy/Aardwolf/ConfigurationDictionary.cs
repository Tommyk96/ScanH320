    ...
    public sealed class ConfigurationDictionary
    {
        ...
        public string SingleValue(string key)
        {
            ...
            if (!_values.TryGetValue(key, out list) || list.Count == 0)
                throw new Exception(String.Format(CultureInfo.InvariantCulture, "Configuration key '{0}' is required", key));
            if (list.Count > 1)
                throw new Exception(String.Format(CultureInfo.InvariantCulture, "Configuration key '{0}' has more than one value", key));
            return list[0];
        }
