...
namespace FSerialization
{
    ...
    [DataContract]
    public class FullSerializeServerJob : AggCorobBaseInfo, IBaseJob
    {
...
#pragma warning restore CS0642
        ...
        public object GetTsdJob()
        {
            ...
            FullSerializeTsdJob lag = CreateNewLineAggregateJob();
            //отметить задание как поступившее в работу
            if (JobState == JobStates.Empty || JobState == JobStates.New)
                JobState = JobStates.InWork;
            else if (JobState == JobStates.SendInProgress)
                JobState = JobStates.WaitSend;

            lag.JobState = JobState;
        ...
        public string RemoveBoxAndMarkAsBad(FullSerializeTsdJob lj)
        {
            //найти свободный номер короба
            foreach (FullSerializeBox b in readyBoxes)
            {
        ...
        public FullNumLineAggregateJobInfo GetWorckInfo()
        {
            ...
            //всегда возвращаем 100 так как в данном случае это почти бесконечное количество
            r.BoxAvailable = 1;// readyBoxes.FindAll(x => x.state == NumberState.Доступен).Count;
            ...
            r.PalleteAvailable = 0;// readyPalets.FindAll(x => x.State == NumberState.Доступен).Count;
            return r;
        }
        ...
        public string SendReports(string url, string user, string pass, bool partOfList, bool sendEmpty, int reguestTimeOut, bool repeat)
        {
            ...
            try
            {
                ...
                {
                    //создать отчет
                    FullSerializeReport r = CreateReportA();
                    JobState = JobStates.SendInProgress;
                ...
                {
                    FullSerializeReport sr = OrderaArray.First();
                    string metod = sr.partOfList ? "POST" : "PUT";
                    result = WebUtil.SendReport<FullSerializeReport>(url, user, pass, metod, sr, "TsdAggRep" + DateTime.Now.ToString(" dd HH.mm.ss.fff"), id, reguestTimeOut);
                    if (result != "")
                    {
                        JobState = JobStates.WaitSend;
                        return result;
        ...
        public string GetFuncName() { return "Сериализация"; }
        #endregion

        [DataMember]
        public FullSerialize1cOrder order1C;
        [DataMember]
        ...
        [DataMember]
        private List<FullSerializeReport> OrderaArray = new List<FullSerializeReport>(); //массив отчетов для отправки

        public FullSerializeServerJob() : base()
        {
            JobState = JobStates.Empty;
            meta.state = JobIcon.Default;
            jobType = typeof(FullSerializeServerJob);
        }
        public override string InitJob<T>(T order, string user, string pass)
        {
            FullSerialize1cOrder o = order as FullSerialize1cOrder;
            if (o == null)
            ...
            try
            {
                ...
                formatBoxNumber = o.formatBoxNumber;
        ...
        public bool SaveOrder()
        {
            try
            {
                ...
                {
                    string s = Archive.SerializeJSon<FullSerializeServerJob>(this);
                    tmpFile.Write(s);
                    tmpFile.Close();
                    tmpFile.Dispose();
                }
        ...
        public static FullSerializeServerJob RestoreOrder()
        {
            try
            {
                string fileName = System.IO.Path.GetDirectoryName(
                ...
                if (System.IO.File.Exists(fileName))
                {
                    using (System.IO.TextReader tmpFile = new System.IO.StreamReader(fileName))
                    {
                        ...
                        tmpFile.Close();
                        tmpFile.Dispose();
                        FullSerializeServerJob result = Archive.DeserializeJSon<FullSerializeServerJob>(s);
                        if (result == null)
            ...
            return new FullSerializeServerJob();
        }

        public bool RemoveFromArray(FullSerializeReport r)
        {
            return OrderaArray.Remove(r);
        }

        //НОВАЯ ВЕРСИЯ ОТЧЕТА!!!
        private FullSerializeReport CreateReportA()
        {
            FullSerializeReport r = new FullSerializeReport();
            r.id = id;
            ...
            foreach (FullSerializeBox rb in readyBoxes)
            {
                if (rb.state == NumberState.Верифицирован)
                {
                    ...
                    foreach (ItemNum str in rb.productNumbers)
                        r.Items.Add(new UnitSerializeRep(rb.id, $"21{str.Sn}\u001d93{str.Field93}",rb.date));
        ...
        public BoxCheckresult VerifyBox(FullSerializeBox b, bool removeDublicate)
        {
            ...
            //bool bIn = false;
            bool BoxcodeUnic = formatBoxNumber > 0;
            ...
            //пройти по всем обработанным коробам и посмотреть повторы пачек и номера короба
            foreach (FullSerializeBox br in readyBoxes)
            {
                ...
                //повтор номера короба 
                if (b.boxNumber == br.boxNumber && BoxcodeUnic)
                {
                    result.msg = "Короб с таким номером уже существует. Считайте другой номер короба или отключите проверку номеров!";
                    return result;//throw new Exception("Короб с таким номером уже существует. Считайте другой номер короба или отключите проверку номеров!");
                }
                ...
                if (br.state == NumberState.Верифицирован || br.state == NumberState.VerifyAndPlaceToReport || br.state == NumberState.VerifyAndPlaceToPalete)
                {
                    ...
                    foreach (ItemNum packCode in b.productNumbers)
                    {
                        if (br.IsAlreadyInBox(packCode.Sn))
                        {
                            ...
                            Log.Write($"задание {id}. Повтор номера { packCode} при проверке");
                        }
                    }
        ...
        private FullSerializeTsdJob CreateNewLineAggregateJob()
        {
            FullSerializeTsdJob pl = new FullSerializeTsdJob();
            pl.id = id;
            pl.lotNo = lotNo;
            pl.order1C = new FullSerialize1cOrder();
            pl.order1C.gtin = GTIN;
            //pl.order1C.ExpDate = ExpDate;
            pl.order1C.formatExpDate = formatExpDate;
            // pl.addProdInfo = addProdInfo;
            pl.order1C.formatBoxNumber = formatBoxNumber;
           // pl.order1C.numLabelAtBox = numLabelAtBox;
            ...
            pl.selectedBox = new FullSerializeBox();
            return pl;
        }
        ...
        {
            jb.state = NumberState.Верифицирован;
         
            readyBoxes.Add(jb);
            Log.Write("Оператор id:" + jb.id + " верифицировал короб:" + jb.boxNumber);
        ...
        public FullSerializeBox RemoveProductNumFromAlreadyVerify(string prNum)
        {
            ...
            foreach (FullSerializeBox b in readyBoxes)
            {
                ...
                if (b.IsAlreadyInBox(prNum))
                {
                    b.productNumbers.RemoveAll(x=> x.Sn == prNum);
                    Log.Write($"Короб {prNum} забракован при повторной проверке");
                    return b;
                }
            }
            return null;
        ...
        public bool BoxCancel(FullSerializeBox jb)
        {
            ...
            //удалить короб из массива обработанных или нет?!
            //readyBoxes.RemoveAll(x=>x.boxNumber == jb.boxNumber);

            //если нет ниодного короба вернуть заданию новый вид
        ...
        //public bool PalleteCancel(PartAggSrvBoxNumber jb)
        //{
        //    if (jb == null)
        //        return false;
