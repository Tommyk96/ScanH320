...
namespace Util
{
    public class GsLabelData
    {

        public string GTIN { get; set; }
        public string SerialNumber { get; set; }
        public string ExpiryDate_JJMMDD { get; set; }
        public string PruductIdentificationOfProducer { get; set; }
        public string SerialShippingContainerCode00 { get; set; }
        public string PackagingComponentNumber { get; set; }
        public string field91Key { get; set; }
        //public string field92Sign { get; set; }
        public string field93Sign;
        public string CryptoHash { get; set; }
        //вох ор паллете
        public string Charge_Number_Lot { get; set; }
        public string ProducerDate_JJMMDD { get; set; }
        public string QuantityInParts { get; set; }
  

        public GsLabelData() { }
        public GsLabelData(string codeLine)
        {
            EAN128Parser.HasCheckSum = false;
            //codeLine = codeLine.Replace("[GS]", "\0");
            char a = Convert.ToChar(0x1D);
            string[] arrayData = codeLine.Split(a);

            try
            {
                foreach (string str in arrayData)
                {
                    Dictionary<EAN128Parser.AII, string> obj = EAN128Parser.Parse(str, false);
                    if (obj != null)
                    {
                        foreach (EAN128Parser.AII item in obj.Keys)
                        {
                            switch (item.AI)
                            {
                                case "00":
                                    SerialShippingContainerCode00 = obj[item];
                                    break;
                                case "01":
                                    GTIN = obj[item];
                                    break;
                                case "21":
                                    SerialNumber = obj[item];
                                    break;
                                case "10":
                                    Charge_Number_Lot = obj[item];
                                    break;
                                case "11":
                                    ProducerDate_JJMMDD = obj[item];
                                    break;
                                case "37":
                                    QuantityInParts = obj[item];
                                    break;
                                case "17":
                                    ExpiryDate_JJMMDD = obj[item];
                                    break;
                                case "240":
                                    PruductIdentificationOfProducer = obj[item];
                                    break;
                                case "91":
                                    field91Key = obj[item];
                                    break;
                                case "92":
                                    //field92Sign = obj[item];
                                    CryptoHash = obj[item];
                                    break;
                                case "93":
                                    field93Sign = obj[item];
                                    CryptoHash = obj[item];
                                    break;
                                case "243":
                                    PackagingComponentNumber = obj[item];
                                    break;
                                default:
                                    continue;
                                    //break;
            ...
            {
                ex.ToString();
                //RsMt.Utilite.Log.Write("Не возможно распознать считанные сканером данные\n" + codeLine);

            }
        }
    }

    public class GsLabelBoxData
    {
        ...
        public GsLabelBoxData() { }
        public GsLabelBoxData(string codeLine)
        {
            EAN128Parser.HasCheckSum = false;
            //codeLine = codeLine.Replace("[GS]", "\0");
            char a = Convert.ToChar(0x1D);
    ...
    public static class EAN128Parser
    {
        ...
        private static Dictionary<string, AII> aiiDict = new Dictionary<string, AII>();
        ...
        static EAN128Parser()
        {
            ...
            Add("91", "Company specific", 2, DataType.Alphanumeric, 4, true);
            Add("92", "Company specific", 2, DataType.Alphanumeric, 88, true);
            ...
            minLengthOfAI = 2;// aiiDict.Values.Min(el => el.LengthOfAI);
            maxLengthOfAI = 4;// aiiDict.Values.Max(el => el.LengthOfAI);
        ...
        /// <returns>The different parts of the ean128 quantity</returns>
        public static Dictionary<AII, string> Parse(string data, bool throwException)
        {
            // cut off the EAN128 start quantity 
            ...
            int index = 0;
            // walkk through the EAN128 quantity
            while (index < data.Length)
            {
                // try to get the AI at the current position
                var ai = GetAI(data, ref index, false);
                if (ai == null)
                {
                    if (throwException)
        ...
        /// <summary>
        /// Get the current quantity to the AI
        /// </summary>
        /// <param name="data">The row data from the scanner</param>
        ...
        public static DateTime TryToParce(string value)
        {
            try {
                String dFormat = "yyMMdd";
                return DateTime.ParseExact(value, dFormat, System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None);
